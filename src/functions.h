...
string s_recieve(int fd) {
    int size;
    ssize_t bytes_read = read(fd, &size, sizeof(size));

    // Проверяем, вернул ли read 0, что указывает на закрытие канала
    if (bytes_read == 0) {
        cout << "Client closed connection or no more data available. Exiting s_recieve." << endl;
        return ""; // Возвращаем пустую строку для обработки завершения
    ...
    char c_message[size];
    bytes_read = read(fd, c_message, size);

    // Если чтение завершилось неожиданно, логируем и завершаем
    if (bytes_read <= 0) {
        cout << "Failed to read full message. Exiting s_recieve." << endl;
        return "";
    }
