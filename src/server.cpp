...
int main() {
    ...
    while (true) {
        string message = s_recieve(fd_recv); // читаем из input  файла

        if (message.empty()) {
            cout << "No more messages to process. Exiting main loop." << endl;
            break;
        }
        ...
        if (f_recipient == "create") { // processing a request to create a chat     //rwvewbjknweovlnrkvp
            vector<string> v_cur;
